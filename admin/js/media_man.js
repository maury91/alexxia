/*
	Versione per il pannello amministrativo di ALExxia
		...
		var
						...
						$.each(a.data, function (a, b) {
							...
							if (b) {
								if ((b.t == "f")&&(media_params.show_files)) {
									...
									if (cla == "load")  
										add = "<img src='" + media_params.base_path+dirx + "/" + b.n + "' class='media_man_imm' />";
									else add = "";
			...
			media_man_diag_ok = function() {
				...
				for (i in media_selected) {
					...
					media_selected[i].n=media_params.base_path+media_selected[i].n;
				}
			...
			readFile = function(file,elem) {			
				if (file) {
					...
					reader.onload = function(e) {
						ajax_request({
							type : 'media_man',
							data : {act : 'upl', d : mediam_dir, filename : file.name, filecontent : Base64.encode(e.target.result), uid : params.uid},
							url : '',
							progress : function(e) {
							...
							success : function(r) {
								...
								if (response.success) {
									...
									if (cla == "load")  
										add = "<img src='" +params.base_path+ mediam_dir + "/" + newn + "' class='media_man_imm' />";
									else add = "";
			...
			media_delete = function() {
				...
				if (confirm(l__del)) {
					ajax_request({
						type : 'media_man',
						data : {act : 'del', sel : media_selected, uid : media_params.uid},
						url : '',
						success: function (d) {
							media_dir(main.get(0),mediam_dir);
						}
					...
					if (params==null) {
						...
						if (media_params.uid != -1) {
							...
							ajax_request({
								...
								url : '',
								success : function (d){
									media_params.multiple = (typeof params.multiple == 'undefined')?d.content.multiple:params.multiple;
									media_params.upload = (typeof params.upload == 'undefined')?d.content.upload:params.upload;
									media_params.del = (typeof params.del == 'undefined')?d.content.del:params.del;
									media_params.extensions = params.extensions || d.content.extensions;
									media_params.dir = params.dir || d.content.dir;
									media_params.show_folder = (typeof params.show_folder == 'undefined')?d.content.show_folder:params.show_folder;
									media_params.show_files = (typeof params.show_files == 'undefined')?d.content.show_files:params.show_files;
									media_params.navigable = params.navigable || d.content.navigable;
									media_data_zone.data({'params':media_params,'dir':media_params.dir});
								}
							});
						} else
